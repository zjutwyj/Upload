...
SWFUploadCreator.check = function() {
    var hasFlash = 0;
    var flashVersion = 0;
    var isIE = /*@cc_on!@*/
    0;
...
SWFUploadCreator.create = function(b) {
    var h;
    var d = {
        flash_url:"/mviews/upload/swfupload.swf",
    ...
    if (typeof Fai != "undefined" && typeof Fai.top._resRoot != "undefined") {
        i = Fai.top._resRoot;
        d.flash_url = "/res" + d.flash_url;
    }
    if (b.button_image_type) {
        var a = b.button_image_type, e = i + "/image/comm/swfupload/swfuploadbtn" + a + ".png";
        $.extend(d, {
            button_image_url:e
        });
    } else {
        $.extend(d, {
            button_image_url:i + "/image/comm/swfupload/swfuploadbtn1.png"
        });
    }
    if (b.view_progress) {
        d.upload_progress_handler = function(j, m, l) {
            var k = 0;
            if (l != 0) {
                k = Math.floor(m / l * 100);
            }
            if (k >= 100) {
                k = 99;
            }
            b.view_progress(j, m, l, k);
        };
    }
    var f = SWFUploadCreator.check();
    if (!f.f) {
        if (b.button_placeholder_id) {
            var c = $("#" + b.button_placeholder_id), g = c.parent();
            g.append("<input type='button' class='swf-btn' value='鏂囦欢涓婁紶' style='width:71px; height:24px;' />");
            c.remove();
            g.find("input.swf-btn").click(function() {
                alert("鎮ㄥ皻鏈畨瑁協lash锛岀偣鍑荤‘瀹氬畨瑁呫€�");
