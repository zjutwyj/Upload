...
function createImage(src,id,option){
    ...
    container.setAttribute("title","请点击下面的名称选取！");
    //设置选中
    ...
    containerSpan.appendChild($(str).get(0));
    return container;
}
...
function popupAddProp() {
    art.dialog.prompt('请输入文件夹名称', function(data, topWin) {
        ...
        $.ajax({
            type : "post",
            url : "/mviews/flash/albumajaxAdd.action",
            data : {
                name : paramName,
                adesc : ''
            },
...
function loadSrcResFilesList(){
    ...
    $.ajax({
        type	: "post",
        url		: "/mviews/flash/albumajaxopen",
        data	: {},
        cache	: false,
        success	: function(result){
        ...
        if(pageType =='xheditor'){
            ...
            $.each(alubmArr,function(){
                if(this.src.indexOf(".flv")>-1){
                    str = '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,29,0"  width="78" height="80">'+
                        '<param name="movie" value="http://img.jihui88.com/vcastr22.swf?vcastr_file='+this.src+'&LogoText=&IsAutoPlay=0">'+
                        '<param name="quality" value="high">'+
                        '<param name="wmode" value="transparent" />'+
                        '<param name="allowFullScreen" value="true" />'+
                        '<embed src="http://img.jihui88.com/vcastr22.swf?vcastr_file='+this.src+'&LogoText=&IsAutoPlay=0" allowFullScreen="true" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" width="78" height="80"></embed>'+
                        '</object>';
                }
                else{
                    str = "<embed width=\"78\" height=\"80\" allowscriptaccess=\"always\" wmode=\"transparent\" src=\""+this.src+"\">";
                }
            });
            callback(str);
        }
